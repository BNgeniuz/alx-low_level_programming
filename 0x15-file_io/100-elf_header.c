...
/**
 * check_elf - fin out if file is an ELF file.
 * @e_ident: array ptr containing the ELF magic num
 *
 * Description: If not an ELF file - exit code with 98.
 */
void check_elf(unsigned char *e_ident)
{
	int num;
...
/**
 * print_magic - funtion that print magic numb of an ELF header.
 * @e_ident: array ptr containing the ELF magic numb
 *
 * Description: separate magic num by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int num;
 ...
 * @e_ident: array ptr containing the ELF field
 */
void print_class(unsigned char *e_ident)
{
	printf(" Class: ");
 ...
 * @e_ident: array ptr containing the ELF version.
 */
void print_version(unsigned char *e_ident)
{
	printf(" Version: %d",
...
/**
 * print_abi - fnxtn to print ABI version of an ELF header.
 * @e_ident: array ptr containing the ELF ABI version.
 */
void print_abi(unsigned char *e_ident)
{
 ...
 * @e_entry: addrs of the ELF entry point.
 * @e_ident: array ptr containing the ELF class.
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
...
/**
 * close_elf - program to close an ELF file.
 * @elf: an ELF file descriptor
 *
 * Description: If cannot be closed - exit code with 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	reads = read(original, header, sizeof(Elf64_Ehdr));
	if (reads == -1)
	{
		free(header);
		close_elf(original);
